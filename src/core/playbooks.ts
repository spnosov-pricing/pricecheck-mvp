...
import type { PricingPlaybook, PricingTier, PlaybookTip } from './types';

const SAAS_TIERS: PricingTier[] = [
   {
      name: 'Starter',
      features: ['Core functionality', 'Up to 5 users'],
      priceRange: '$29-49/mo',
...
const SAAS_TIPS: PlaybookTip[] = [
   {
      category: 'Psychology',
      advice: 'Use $99 instead of $100 for conversion',
      source: 'Bain',
   },
   {
      category: 'Strategy',
      advice: 'Align price with value metrics, not costs',
      source: 'McKinsey',
   },
];
...
export const getPlaybookByCategory = (cat: string) =>
   PRICING_PLAYBOOKS.find((p) => p.category === cat);
